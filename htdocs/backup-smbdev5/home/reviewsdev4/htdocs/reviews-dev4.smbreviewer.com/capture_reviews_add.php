...
if (isset($row['short_io_api_key']) && trim($row['short_io_api_key']) != "") {
    ...
    if (isset($row['short_io_domain']) && trim($row['short_io_domain']) != "") {
        ...
        } elseif ($short_io_domain_link['error'] === "Link not found") {
...
$questions = $db->getCaptureReviewsQuestionsByReviewId((int)$_REQUEST['id']);

if ($row['review_mode']=='multiple' && !empty($questions)) {
$default_questions['multiple']['text'] =  $questions;
} else {
$default_questions['multiple']['text'] = [
                                           [   'order'=>1, 'text'=>'How did you find us?'],
                                           [   'order'=>2, 'text'=>'Describe your personal experience with our brand?'],
                                           [   'order'=>3, 'text'=>'What do you love about our products or services?']
                                         ];
}


$default_questions['multiple']['video'] = $default_questions['multiple']['text'];


if ($row['review_mode']=='single' && $row['type']=='text' &&  !empty($questions)) {
$default_questions['single']['text'] = $questions;
} else {

$default_questions['single']['text'] = [
                                           [   'order'=>1, 'text'=>'Your Review:']
                                         ];
}

if ($row['review_mode']=='single' && $row['type']=='video' &&  !empty($questions)) {
$default_questions['single']['video'] = $questions;
} else {

$default_questions['single']['video'] = [
                                          [   'order'=>1, 'text'=>'Additional comments:']
                                        ];

}

// error_log(print_r($default_questions[$row['review_mode']][$row['type']],true));
...
<style media="screen">
  ...
  background: white;
    ...
    #btn_enable_google_sheets span.icon {

      background-image: url("/assets/images/btn_google_light_normal_ios.svg");
      display: inline-block;
    ...
    #btn_enable_google_sheets span.text {
      display: inline-block;
     vertical-align: middle;
     /* padding-left: 42px; */
     /* padding-right: 42px; */
            ...
            <section class="panel">
                <div class="panel-body">
                    ...
                    <form id="capture_reviews" action="/action.php?action=capture_reviews" class="form-horizontal form-bordered" method="post" enctype="multipart/form-data">
                        ...
                        <!-- Review mode  -->
                        <div class="form-group">
                            <label class="col-md-3 control-label">Review mode</label>
                            <div class="col-md-6 pt-2" style="text-align:left">
                                <input type="radio" name="review-mode" id="single-question-review" value="single" <?= (isset($row['review_mode']) && $row['review_mode'] == 'single' ? 'checked' : '') ?>>
                                <label for="single-question-review">Single question review</label>
                                <input type="radio" name="review-mode" id="multiple-question-review" value="multiple" style="margin-left: 20px" <?= (isset($row['review_mode']) && $row['review_mode'] == 'multiple' ? 'checked' : '') ?>>
                                <label for="multiple-question-review">Multiple questions review</label>
                            </div>
                            <div class="clearfix"></div>

                            <div class="" id="questions">
                                <?php foreach ((empty($questions) ? $default_questions[$row['review_mode']][$row['type']] : $questions) as $q): ?>
                                  <div class="question-template">
                                    <label class="col-md-3 control-label">#<?=$q['order']?></label>
                                    <div class="col-md-9 pb-4" >
                                      <textarea class="form-control" name="review_questions[<?=$q['order']?>]"><?=$q['text']?></textarea>
                                    </div>
                                    <div class="clearfix"></div>
                                  </div>
                                <?php endforeach; ?>
                          ...
                          <div class="col-md-9 col-md-offset-3 questions-info <?= (isset($row['review_mode']) && $row['review_mode'] == 'multiple' ? '' : 'hidden') ?>" >
                            <em>The the reviewer will be asked a set of questions (up to 3), after the reviewer clicks "submit," the answers will be combined and the user can paste their review in the directory of your choice.</em>
                          </div>
                        ...
                        <!-- Type of share -->
                        <div class="form-group">
                            <label class="col-md-3 control-label">Select type of review sharing</label>
                            <div class="col-md-6 d-flex">
                                <label class="radio-inline" for="review-share-type-s"><input type="radio" value="share_buttons" name="review-share-type" id="review-share-type-s" <?= ((isset($row['type_of_share']) && ($row['type_of_share'] == 'share_buttons' || trim($row['type_of_share']) == ''))? 'checked' : '') ?> >Share review link on socials</label>
                                <label class="radio-inline" for="review-share-type-d"><input type="radio" value="directories" name="review-share-type" id="review-share-type-d" <?= (isset($row['type_of_share']) && $row['type_of_share'] == 'directories' ? 'checked' : '') ?> >Post review to directories</label>
                              </div>
                        </div>

                        <!-- Share on social media section  -->
                        <div class="form-group <?= ((isset($row['type_of_share']) && ($row['type_of_share'] == 'share_buttons' || trim($row['type_of_share']) == ''))? '' : 'hidden') ?>" id="social-media-share" >
                        ...
                        <!-- Share to directory section  -->
                        <div class="form-group <?= (isset($row['type_of_share']) && $row['type_of_share'] == 'directories' ? '' : 'hidden') ?> " style="text-align:left;" id="directories-share">
                        ...
                        <!-- end of Share to directory section  -->
 ...
 e.g. https://www.youtube.com/embed/c_pBRQVKoLE" data-container="body">
                        ...
                        <div class="form-group ">
                            ...
                            <?php if ($db->isUserCurrentPlanDefault($_SESSION['user_id']) == false) : ?>
                                ...
                                <div class="col-md-6">
                                  <?php if ((int)$row['enable_google_sheets'] == 0): ?>
                                    <div id="btn_enable_google_sheets">
                                      <span class="icon"></span>
                                      <span class="text">Sign in with Google</span>
                                    </div>
                                      <div class="input-group mb-sm hidden" >
                                        <?php else: ?>
                                      <div class="input-group mb-sm ">
                                  <?php endif; ?>
                                        ...
                                        <div class="input-group-addon">
                                          ...
                                          <input type="<?= ((int)$row['enable_google_sheets'] == 0 ? "hidden" : "checkbox") ?>" class="" name="enable_google_sheets" value="<?= $row['enable_google_sheets'] ?>" <?= ((int)$row['enable_google_sheets'] == 0 ? "" : " checked") ?>>
                                        </div>
                        ...
                        <!-- this was a place of directory settings  -->
                            ...
                            <div class="col-md-4">
                                <?php if (isset($row['short_io_api_key']) && $row['short_io_api_key'] != '') : ?>
                                    <div class="input-group" id="short-io-group">
                                        ...
                                        <div id="short-io-container" style="display: flex; flex-direction: column;">
                                            <?php if (isset($row['short_io_api_key']) && $row['short_io_api_key'] != "") : ?>
                                            ...
                                            <div style="display: flex; justify-content: space-between">
                                                <a target="_blank" class="form-control <?= ($short_io_domain_link == "" ? "hidden" : "") ?>" id="short_io_domain_link" href="<?= $short_io_domain_link ?>" data-toggle="tooltip" data-placement="top" title="Short.io link">
                                                    <?= $short_io_domain_link ?>
                                                    
                                                </a>
                                                <div id="short_io_domain_link_copy" class="input-group-addon" data-toggle="tooltip" data-placement="top" title="copy link to clipboard" style="width: 40px; display: flex; align-items: center; justify-content: center; cursor: pointer">
                        ...
                        <div class="form-group d-flex align-items-center">
                            <label class="col-md-3 control-label">
                                ...
                                <a data-toggle="popover" data-html="true" data-content="When the user scans this QR Code, it will redirect to the campaign review form." data-container="body">
                                    <i class="fa fa-question-circle " aria-hidden="true"></i>
                                </a>

                            </label>
                            <div id="qr__code--area" class="col-md-6">
                                <div>
                                    <?php if($row['qr_code'] != null) : ?>
                                    <?php $url = encodeURIComponent($row['qr_code']) ?>
                                        <a id="qr__code--link" href="/action.php?action=download_qr_code&url=<?php $url ?>">
                                            <img id="qr__code--img" src="<?= $row['qr_code']; ?>" width="100px" height="100px"/>
                                        </a>
                                        
                                        <div style="margin: 10px;">
                                            <a onclick="PrintImage('<?=$row['qr_code']?>')" class="btn btn-primary mr-3">
                                                <i class="fa fa-print"></i>
                                            </a>
                                            <a href="/action.php?action=download_qr_code&url=<?=$url?>" class="btn btn-info">
                                                <i class="fa fa-download"></i>
                                            </a>
                                        </div>
                                    <? else : ?>
                                        <h4>This campaign hasn't a QR Code. Please update to create QR Code</h4>
                                    <? endif ?>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                ...
                                <button type="submit" name="submit" value="submit" class="btn  btn-fill btn-success"
                                    onclick="submitFormCaptureReviewAdd()">
...
<!-- <script src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.6.3/js/all.min.js"></script> -->
<script>
    $(document).on('click', '#short_io_domain_link_copy',function() {
        console.log('copping ');
        var $temp = $("<input>");
        $("body").append($temp);
        $temp.val($('#short_io_domain_link').text()).select();
        console.log($temp);
        $temp.focus();
...
<script type="text/javascript">
    ...
    let defaultQuestions  = <?= json_encode($default_questions, true) ?>;

    let selectedTemplate = <?= isset($row['review_template_id'])&&$db->getReviewTemplate($row['review_template_id'])!=null ? json_encode($db->getReviewTemplate($row['review_template_id']), true) : json_encode(reset($templates), true) ?>
    ...
    function setTemplate(template) {
        ...
        $('#template-preview').attr('src', '/uploads/preview_images/' + encodeURIComponent(template.preview_image));
    ...
    function submitFormCaptureReviewAdd(){
        document.forms['capture_reviews'].submit();
    }
    ...
    $(document).ready(function() {
        ...
        // share only for video type - now for all types
        //
        ...
        $('input[name="review-share-type"]').change(function(){
          switch ($(this).val()) {
            case 'share_buttons':
              // $( '#directories-share input[type="checkbox"]' ).each(function() {
              //
              //   $(this).removeAttr('checked').val(0);
              //
              // });
              $('#directories-share').addClass('hidden');
              $('#social-media-share').removeClass('hidden');
              break;

            case 'directories':

              // $( '#social-media-share input[type="checkbox"]' ).each(function() {
              //
              //   $(this).removeAttr('checked').val(0);
              //
              // });
              $('#social-media-share').addClass('hidden');
              $('#directories-share').removeClass('hidden');
        ...
        $('input[name="review-mode"]').change(function(){
          var textArea_templ=$('#questions .question-template').first();

          $('#questions .question-template').remove();

          // console.log($(this).val(),$('input[name="review-type"]:checked').val());
          $.each(defaultQuestions[$(this).val()][$('input[name="review-type"]:checked').val()], function(k, v) {

            var a = textArea_templ.clone();
            a.find('label').html('#'+v.order);
            a.find('textarea').attr('name','review_questions['+v.order+']').val(v.text);
            $('#questions').append(a);
          ...
          switch ($(this).val()) {
            case 'single':
              ...
              $('.questions-info').addClass('hidden');

              break;

            case 'multiple':
               $('.questions-info').removeClass('hidden');
        ...
        $('#multiple-questions textarea').summernote({
            toolbar: [
                ['font', ['bold', 'underline', 'clear']],
                ['fontname', ['fontname']],
                ['color', ['color']],
        ...
        $('#btn_enable_google_sheets').on("click",function() {


            // $('input[name="enable_google_sheets"]').attr("checked",true).val('1');
            $('input[name="enable_google_sheets"]').attr("checked",true).val(1);
            
            $('input[name="enable_google_sheets"]').trigger('change');
                                ...
                                $.post("/action.php", {
                                    ...
                                    function(data) {
                                        ...
                                        if (data.result.google_access_token.trim() != '') {
                                          ...
                                          $('#btn_enable_google_sheets').hide();
                                          $('input[name="enable_google_sheets"]').attr('type','checkbox').closest('.input-group.hidden').removeClass('hidden');

                                            $('input[name="enable_google_sheets"]').val(1);
                                            $('#set_google_sheet_id').removeClass('disabled');
                                            $('input[name="google_spread_sheet_id"]').attr('disabled', false);
                                        } else {
                                            $('input[name="enable_google_sheets"]').attr('checked', false);
                                        }
                                    });
                            }
                        }, 500);
