...
$questions = $db->getCaptureReviewsQuestionsByReviewId($capture_reviews_id);


if ($data['review_mode']=='multiple' && !empty($questions)) {
...
if ($data['review_mode']=='single' && $data['type']=='text' &&  !empty($questions)) {
...
if ($data['review_mode']=='single' && $data['type']=='video' &&  !empty($questions)) {
...
$user = $db->getUserProfile($data["user_id"]);
// 1 is free
// 3 premium
$planId = $user["plan_id"];

$canRecordReview = true;
if ($nReviews["numberOfReviews"] >= 15 && $user["plan_id"] == 1) {
...
$template_footer = "
<script>
  let questions = ".json_encode($default_questions[$data['review_mode']][$data['type']],true).";
...
<script type='text/javascript' src='/uploads/templates/template_functions.js?v=".time()."'></script>
";
...
if (isset($_REQUEST['preview']) && $_REQUEST['preview'] == true) {
  ...
  if ($template==null) {
     $template = $db->getFirstLiveReviewTemplate();
   }
...
//if(is_file(dirname(__FILE__) . $basePath . '/' . $template['template'])) {
//include_once(dirname(__FILE__) . $basePath . '/' . $template['template']);
//}

// BLADE 4.X PHP 8.0
//$renderer = new Windwalker\Renderer\BladeRenderer([__DIR__ . '/uploads/templates/' . $template['name']], ['cache_path' => __DIR__ . '/cache/templates']);
...
require_once(__DIR__ .'/core/vendor/edge-3.x/vendor/autoload.php');

$loader = new \Windwalker\Edge\Loader\EdgeFileLoader([__DIR__ . '/uploads/templates/' . $template['name']]);
$loader->addFileExtension('.php');
$edge = new \Windwalker\Edge\Edge($loader, null, new Windwalker\Edge\Cache\EdgeFileCache(__DIR__ . '/cache/templates'));
//$edge->addGlobal('data',$data);
$compact = [];
foreach (['data', 'default_questions', 'form_token', 'template_footer', 'canRecordReview'] as $i) if (isset(${$i})) $compact[$i] = ${$i};
echo $edge->render(preg_replace('/\.php$/i', '', $template['template']), $compact);