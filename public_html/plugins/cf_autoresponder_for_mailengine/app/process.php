...
class Cfautores_processor
{
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            if($autores=='mailengine')
            {
              register_autoresponder($id,$title,function($data,$arg2){
                $file=plugin_dir_path(__FILE__);
              $file .="mailengine/mailengine.php";
              require_once($file);

              $ob=new \CFAuto_mailengine_autores();
              $tocheck=$ob->Mailengine($arg2[0],$data['email'],$data['name']);
                },array($credentials));  
              } 
              else
              {
    ...
    {  
        ...
        $date=time();
            if($autotype=='mailengine')
            {
              ...
              require_once($file);
              $ob=new \CFAuto_mailengine_autores();
              $tocheck_autores=$ob->Mailengine($credentials,$email,'test');
            }
            if($tocheck_autores)
            {
                if($id<1)
