...
class Cfautores_processor
{
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            if($autores=='constantcont')
            {
              register_autoresponder($id,$title,function($data,$arg2){
                $file=plugin_dir_path(__FILE__);
              $file .="constantcont/constantcont.php";
              require_once($file);

              $ob=new \CFAuto_constantcont_autores();
              $tocheck=$ob->Mailerlite($arg2[0],$data['email'],$data['name']);
                },array($credentials));  
              } 
              else
              {
    ...
    {  
        ...
        foreach($credentials_j as $key => $cred)
        {
          ...
          $decocredentials = json_decode($credentials);
         $apikey =$decocredentials->apikey;
         $access_token =$decocredentials->accesstoken;
         $listid = $decocredentials->listid;
        $exf = json_encode(array("name"=>"test","email"=>$email));
        $date=time();
            if($autotype=='constantcont')
            {
