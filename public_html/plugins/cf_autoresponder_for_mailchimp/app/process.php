...
class Cfautores_processor
{
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            if($autores=='mailchimp')
            {
              register_autoresponder($id,$title,function($data,$arg2){
                $file=plugin_dir_path(__FILE__);
              $file .="mailchimp/mailchimp.php";
              require_once($file);

              $ob=new \CFAuto_mailchimp_autores();
              $tocheck=$ob->Mailchimp($arg2[0],$data['email'],$data['name']);
                },array($credentials));  
              } 
              else
              {
    ...
    {  
        ...
        $date=time();
            if($autotype=='mailchimp')
            {
              $file=plugin_dir_path(__FILE__);
              $file .="mailchimp/mailchimp.php";
              require_once($file);
              $ob=new \CFAuto_mailchimp_autores();
              $tocheck_autores=$ob->Mailchimp($credentials,$email,'test');
            }
            if($tocheck_autores)
            {
                if($id<1)
