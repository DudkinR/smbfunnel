<?php
namespace CFPAY_peyment_addon\fedapay;
class Cfpay_processor
{
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            function($data, $product, $call_back_url){
                ...
                if($data['method']=='fedapay')
                {
                    
                    $file=plugin_dir_path(__FILE__);
                     $file .="fedapay/fedapay_payment.php";
                    require_once($file);
                    $ob=new \CFPay_fedapay_payment();
    ...
    function doSaveUpdateSetup( $data )
    {
        ...
        $id=(int)$id;
        $title=$mysqli->real_escape_string($data['fedapay_title']);
        $public_key=$mysqli->real_escape_string($data['fedapay_public_key']);
        $secret_key=$mysqli->real_escape_string($data['fedapay_secret_key']);
        $tax=$mysqli->real_escape_string($data['fedapay_tax']);
        $type=$mysqli->real_escape_string($data['fedapay_type']);
        $method=$this->method;
