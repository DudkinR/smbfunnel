<?php
// namespace CFAUTO_autoresponder_addon\activecampaign;
class Cfautores_activecampaign_process
{
    var $pref;
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            if($autores=='activecampaign')
            {
              register_autoresponder($id,$title,function($data,$arg2){
                $file=plugin_dir_path(__FILE__);
               $file .="activecampaign/activecampaign.php";
              require_once($file);

              $ob=new \Cfautores_activecampaign_processor();
              $tocheck=$ob->Activecampaign($arg2[0],$data['email'],$data['name']);
                },array($credentials));  
              } 
              else
              {
    ...
    function doSaveUpdateSetup($id,$title,$autotype,$credentials_js)
    {  
        global $mysqli;
        global $dbpref;
        $table= $dbpref.'quick_autoresponders';
        ...
        $jsondecode = json_decode($credentials);
        ...
        if($autotype=='activecampaign')
        {
            ...
            $dir.="activecampaign/api/includes/ActiveCampaign.class.php";
            require $dir;
            $ac = new ActiveCampaign($apiurl,$apikey);
            if ($ac->credentials_test() == 1) {
                if($id<1)
                {
                    $sql="INSERT INTO `".$table."` (`autoresponder`, `autoresponder_name`, `autoresponder_detail`, `exf`, `date_created`) VALUES ('".$title."','".$autotype."','".$credentials."','','".$date."')";
                }
                else
                {
                    $sql = "UPDATE `".$table."` set `autoresponder`='".$title."',`autoresponder_name`='".$autotype."',`autoresponder_detail`='".$credentials."',`exf`='' where `id`='".$id."'";
                }
    ...
    function registerAjaxRequest(){
        ...
        echo self::doSaveUpdateSetup($_POST['cfautores_id'],$_POST['cfautores_title'],$_POST['cfautores_method'],$_POST['cfautores_credentials']);
            die();
        });
    }   
}
