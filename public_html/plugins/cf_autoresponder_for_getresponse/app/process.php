...
class Cfautores_processor
{
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            if($autores=='getresponse')
            {
              register_autoresponder($id,$title,function($data,$arg2){
                $file=plugin_dir_path(__FILE__);
              $file .="getresponse/getresponse.php";
              require_once($file);

              $ob=new \CFAuto_getresponse_autores();
              $tocheck=$ob->Getresponse($arg2[0],$data['email'],$data['name']);
                },array($credentials));  
              } 
              else
              {
    ...
    {  
        ...
        $date=time();
            if($autotype=='getresponse')
            {
              $file=plugin_dir_path(__FILE__);
              $file .="getresponse/getresponse.php";
              require_once($file);
              $ob=new \CFAuto_getresponse_autores();
              $tocheck_autores=$ob->Getresponse($credentials,$email,'test');
            }
            if($tocheck_autores)
            {
                if($id<1)
