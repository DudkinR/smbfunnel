    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            function($data, $product, $call_back_url){
                ...
                if($data['method']=='2checkout')
                {
                    $file=plugin_dir_path(__FILE__);
                    $file .="2checkout/Cfpayment_two_checkout.php";
                    require_once($file);
                    $ob=new \Cfpayment_two_checkout();
                    return $ob->doPayment($data,$product);
                }
                /*
                else if($data['method']=='paystack')
                {
                    $file=plugin_dir_path(__FILE__);
