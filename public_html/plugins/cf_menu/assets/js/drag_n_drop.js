function factory($) {
    ...
    Sortable.prototype.serialize = function(container) {
        ...
        return container.children(self.options.nodes).not(self.options.container).map(function(ix, el) {
            var $el = $(el),
                text = $el.clone().children().html().trim(), //text only without children
                id = $el.attr('id'),
                getClass = $el.attr('class'),
                url = $el.clone().children('div').attr('a-source'),
                icon = $el.clone().children('div').attr('a-icon'),
                extra_ids = $el.clone().children('div').attr('a-extraIds'),
                direct_id = $el.clone().children('div').attr('direct_id'),
                node = { 'name': id ||  text, 'url': url, 'a-extraIds': extra_ids, 'direct_id': direct_id, 'class': getClass, 'icon': icon };
