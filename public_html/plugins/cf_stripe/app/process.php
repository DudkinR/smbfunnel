...
class Cfpay_processor
{
    ...
    function loadSetting($id)
    {
        ...
        $sql = $mysqli->query("SELECT * FROM `$table` WHERE `stripe_id`=$id");
        if( $sql->num_rows > 0 )
        {
    ...
    function registerAjaxRequest(){
        ...
        add_action('cf_ajax_cfstripe_billing_btn',function(){
            echo self::doSaveUpdateSetting($_POST);
                die();
        });
    }
    function doSaveUpdateSetting( $data)
    {
        global $mysqli;
        global $dbpref;
        $table=$dbpref."stripe_setting";
        $id = (int)$mysqli->real_escape_string($data['stripe_id']);
        $body =$mysqli->real_escape_string( $data['file_content'] ); 
        $subject=$mysqli->real_escape_string( $data['file_subject'] );
        $return_url=$mysqli->real_escape_string( $data['return_url'] );
        $email_days=$mysqli->real_escape_string( $data['email_days'] );
        update_option("cfstripe_reminder_days",$email_days);
        $card_html='';
        $card_html=$mysqli->real_escape_string( $data['card_html'] );
                ...
                foreach($setting as $kk=>$ss)
                {
                    if(is_array($ss))
                    {
                        foreach($ss as $k=>$s)
                        {
                            ...
                            $fdata[$mysqli->real_escape_string($key)][$mysqli->real_escape_string($kk)][$mysqli->real_escape_string($k)] = $mysqli->real_escape_string($s);
                        }

                    }else{
                        $fdata[$mysqli->real_escape_string($key)][$mysqli->real_escape_string($kk)] = $mysqli->real_escape_string($ss);
                    }
                }

            }else{
                $fdata[$mysqli->real_escape_string($key)] = $mysqli->real_escape_string($setting);
            }
        }
        $add_setting=$mysqli->real_escape_string(json_encode($fdata));
        $run = $mysqli->query("SELECT `id` FROM `$table` WHERE `stripe_id`=$id");
        if( $run->num_rows > 0)
        {
            $run1 = $mysqli->query("UPDATE `$table` SET `setting`='$add_setting', `email_subject`='$subject',`email_content`='$body',`card_html`='$card_html',`return_url`='$return_url',`email_days`='$email_days' WHERE `stripe_id`=$id");
            // echo "UPDATE `$table` SET `setting`='$add_setting', `email_subject`='$subject',`email_content`='$body' WHERE `stripe_id`=$id";
            if($run1)
            {
                return json_encode(['status'=>true,'message'=>'udpated']);
            }else{
            ...
            $user_id=$_SESSION['user' . get_option('site_token')]; 
            //$access=$_SESSION['access' . get_option('site_token')]; 
            $run1 = $mysqli->query("INSERT INTO `$table` (`setting`,`stripe_id`,`email_subject`,`email_content`,`card_html`,`return_url`,`email_days`,`user_id`) VALUES ('$add_setting', $id,'$subject','$body','$card_html','$return_url','$email_days','$user_id')");
            if($run1)
            {
