...
class Cfautores_processor
{
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            if($autores=='convertkit')
            {
              register_autoresponder($id,$title,function($data,$arg2){
                $file=plugin_dir_path(__FILE__);
              $file .="convertkit/convertkit.php";
              require_once($file);

              $ob=new \CFAuto_convertkit_autores();
              $tocheck=$ob->convertkit($arg2[0],$data['email'],$data['name']);
                },array($credentials));  
              } 
              else
              {
    ...
    {  
     ...
     $decode_data = json_decode($credentials);
    //   $secret_id = $decode_data->secret_id;
       $formid = $decode_data->listid;
       $exf = json_encode(array("name"=>"test","email"=>$email));
        $date=time();
            if($autotype=='convertkit')
            {
                ...
                $ob=new \CFAuto_convertkit_autores();
                 $tocheck_autores=$ob->convertkit($credentials,$email,'test'); 

            }
            if($tocheck_autores == "200")
            {
                if($id<1)
                {
