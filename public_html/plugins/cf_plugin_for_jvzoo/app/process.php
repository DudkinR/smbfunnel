...
class Cfpay_processor
{
    ...
    function loadMethods()
    {
        ...
        {
            $r->method= str_replace('_ipn', '', $r->method);
            ...
            function($data, $product, $call_back_url){
                ...
                if($data['method']=='jvzoo_ipn')
                {
                    $file=plugin_dir_path(__FILE__);
                    $file .="jvzoo/JVZoo_payment.php";
                    require_once($file);
                    $ob=new \CFPay_JVZoo_payment();
    ...
    function countSetups()
    {
        ...
        {
            $qry=$mysqli->query("select count(`id`) as `count_id` from `".$table."` where `method` in ('".$method."', '".$method."_ipn') and `user_id`=".$user_id."");
        }
        if($qry->num_rows>0)
        {
            $r=$qry->fetch_object();
    ...
    function loadSetups($page=1)
    {
        ...
        {
            $qry=$mysqli->query("select * from `".$table."` where `method` in ('".$method."', '".$method."_ipn') and `user_id`=".$user_id."".$search." order by ".$order_by.$limit_str);
        }
        while($r=$qry->fetch_object())
        {
            array_push($arr,$r);
