...
class Cfautores_processor
{
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            if($autores=='mailwizz')
            {
              ...
              $file .="mailwizz/mailwizz.php";
              require_once($file);

              $ob=new \CFAuto_mailwizz_autores();
              $tocheck=$ob->mailwizz($arg2[0],$data['email'],$data['name'],false);
                },array($credentials));  
              } 
              else
              {
    ...
    function loadSetups($page=1)
    {
        ...
        if($access=='admin')
        {
            $sql_text="SELECT * FROM `".$table."`  WHERE `autoresponder_name`='".$autores."' AND 1 ".$search." ORDER BY ".$order_by.$limit_str;
        }
        else
        {
            $sql_text="SELECT * FROM `".$table."`  WHERE `autoresponder_name`='".$autores."' AND `user_id`='".$user_id."' ".$search." ORDER BY ".$order_by.$limit_str;
        }
    ...
    {  
        ...
        $date=time();
            if($autotype=='mailwizz')
            {
              ...
              require_once($file);
              $ob=new \CFAuto_mailwizz_autores();
              $tocheck_autores=$ob->mailwizz($credentials,$email,'test',false);
            }
            if($tocheck_autores)
            {
                if($id<1)
