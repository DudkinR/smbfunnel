...
use CFPAY_peyment_addon\bkash as CFPAY_bkash;

if(!class_exists('Cfpaymentaddon_index'))
{
    require_once('app/process.php');
    class Cfpaymentaddon_index extends CFPAY_bkash\Cfpay_processor
    {
        var $pref='cfpay';
        var $config=false;
        var $method='bkash';
        ...
        {    do_session_start();
            ...
            add_action('admin_menu',function(){
                $logo_url=plugins_url('assets/img/bkash.ico',__FILE__);
                add_menu_page('Bkash','Bkash','cfpay_setups_'.$this->method,array($this,'createMenu'),$logo_url,'All Setups');
                add_submenu_page('cfpay_setups_'.$this->method,'Manage Setup(Bkash)','Create New','cfpay_setting_'.$this->method,function(){
    ...
    new \CFPAY_peyment_addon\bkash\Cfpaymentaddon_index();
}
?>