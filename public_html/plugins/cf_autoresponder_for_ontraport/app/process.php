...
class Cfautores_processor
{
    ...
    function loadMethods()
    {
        ...
        while($r=$qry->fetch_object())
        {
            ...
            if($autores=='ontraport')
            {
              register_autoresponder($id,$title,function($data,$arg2){
                $file=plugin_dir_path(__FILE__);
              $file .="ontraport/ontraport.php";
              require_once($file);

              $ob=new \CFAuto_ontraport_autores();
              $tocheck=$ob->Ontraport($arg2[0],$data['email'],$data['name']);
                },array($credentials));  
              } 
              else
              {
    ...
    {  
       ...
       $appid = $jsondecode->appid;
        ...
        $date=time();
            if($autotype=='ontraport')
            {
                include('ontraport/api/src/Ontraport.php');
