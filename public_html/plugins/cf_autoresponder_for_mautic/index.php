<?php
namespace CFAuto_autoresponder_addon\mautic ;
use CFAuto_autoresponder_addon\mautic as CFAUTO_mautic;

if(!class_exists('Cfautoresaddon_index'))
{
    require_once('app/process.php');
    class Cfautoresaddon_index extends CFAUTO_mautic\Cfautores_processor
    {
        var $pref='cfautores';
        var $config=false;
        var $autores='mautic';
        ...
        function createMenuandSubmenu()
        {
            ...
            add_action('admin_menu',function(){
                ...
                add_menu_page('Mautic','Mautic','cfautores_setups_'.$this->autores,array($this,'createMenu'),$logo_url,'All Setups');
                add_submenu_page('cfautores_setups_'.$this->autores,'Manage Setup(Mautic)','Create New','cfautores_setting_'.$this->autores,function(){
            ...
            if(isset($_GET['page']) && in_array($_GET['page'],array('cfautores_setups_'.$this->autores, 'cfautores_setting_'.$this->autores)))
            {
                add_action('admin_head',function(){
                    ...
                    if($_GET['page']=='cfautores_setting_'.$this->autores)
                    {
                        ...
                        echo "<script type='module' src='".plugins_url('/assets/js/customvue.js?v='.$version,__FILE__)."'></script>";
                    }
    ...
    new \CFAuto_autoresponder_addon\mautic\Cfautoresaddon_index();
}
?>